...
(function() {
  ...
  const domGuidesIndex = document.getElementById("guidesIndex");
  ...
  function renderGuides() {
    ...
    if (activeGuide) {
      domGuidesIndex.classList.remove("active");
      if (activeGuide.toc != "") {
        ...
        function onLinkClick(ev) {
          ...
          scrollToHeading(parseInt(link.split(":")[1]), true);
          ev.preventDefault();
          ev.stopPropagation();
        }
      ...
      domGuidesIndex.classList.add("active");
      domGuideTocList.classList.add("hidden");
      
      if (zigAnalysis.guide_sections.length == 0) {
        noGuidesAtAll();
      } else {
        renderGuidesIndex();
      }
  ...
  function renderGuidesIndex() {
    let html = "";
    for (let section of zigAnalysis.guide_sections) {
      if (html == "") {
        // first section is the default section
        html += "<h2>Main Guides</h2>";
      } else {
    ...
    switch (mode) {
      ...
      case NAV_MODES.GUIDES:
...

        {
          ...
          if (parts[1] != null) {
            curNav.activeGuideScrollTo = parseInt(decodeURIComponent(parts[1]));
          }
        }
        return;
      default:
  ...
  function scrollToHeading(index, alreadyThere) {  
    let count = 0;
    for (let c of domActiveGuide.childNodes) {
      if (["H2", "H3", "H4", "H5"].includes(c.tagName)){
        if (count == index) {
          if (alreadyThere) {
