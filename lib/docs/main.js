...
`);
    ...
    if (activeGuide) {
      ...
      while ((event = walker.next())) {
        ...
        if (event.entering) {
          ...
          if (node.type === 'heading') {
            if (node.level == 1) continue; // don't index H1
            node._type = "item";
            if (last_ul) {
            ...
            walker.resumeAt(last, false); // exit right away
          } else {
            if (last) {
              walker.resumeAt(last, false);
            } else {
              walker.resumeAt(doc, true);
            }
          }
        }
      }        
      ...
      domGuideTocList.innerHTML = result;
  ...
  function parseGuides() {
    for (let j = 0; j < zigAnalysis.guide_sections.length; j += 1) {
      ...
      for (let i = 0; i < section.guides.length; i += 1) {
        let reader = new commonmark.Parser({ smart: true });
        const guide = section.guides[i];
        guide.ast = reader.parse(guide.body);        
